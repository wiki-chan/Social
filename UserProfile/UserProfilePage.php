...
//				'<a href="' . htmlspecialchars( $update_profile->getFullURL() ) . '">' . wfMessage( 'user-edit-profile' )->escaped() . '</a>' .
				'<li class="user-profile-menu avatar"><a href="' . htmlspecialchars( $upload_avatar->getFullURL() ) . '" rel="nofollow">'
					. wfMessage( 'user-upload-avatar' )->escaped() . '</a></li>' .
				'<li class="user-profile-menu requests"><a href="' . htmlspecialchars( $view_relationship_requests->getFullURL() ) . '" rel="nofollow">'
					. wfMsg( 'user-requests-title' ) . '</a></li>' .
				'<li class="user-profile-menu bookmarks"><a href="' . htmlspecialchars( $watchlist->getFullURL() ) . '" rel="nofollow">'
					. wfMessage( 'user-watchlist' )->escaped() . '</a></li>' .
				'';
			//) );
		} elseif ( $wgUser->isLoggedIn() ) {
			if ( $relationship == false ) {
				$output .= //$wgLang->pipeList( array(
					'<li class="user-profile-menu follow"><a href="' . htmlspecialchars( $add_relationship->getFullURL( 'user=' . $user_safe . '&rel_type=1' ) ) . '" rel="nofollow">'
						. wfMessage( 'user-add-friend' )->escaped() . '</a></li>'.
				...
				$output .= //$wgLang->pipeList( array(
					'<li class="user-profile-menu unfollow"><a href="' . htmlspecialchars( $remove_relationship->getFullURL( 'user=' . $user_safe ) ) . '" rel="nofollow">'
						. wfMessage( 'user-remove-friend' )->escaped() . '</a></li>'.
					'';
				//) );
			...
			*/
			...
			/* global $wgUserBoard;
			if ( $wgUserBoard ) {
				$output .= '<li class="user-profile-menu "><a href="' . htmlspecialchars( $send_message->getFullURL( 'user=' . $wgUser->getName() . '&conv=' . $user_safe ) ) . '" rel="nofollow"><span class="icon message"></span><span class="caption">' . wfMessage( 'user-send-message' )->escaped() . '</span></a>';
		...
		$output .= '<li class="user-profile-menu sandbox"><a href="' . htmlspecialchars( $sandbox->getFullURL() ) . '" rel="nofollow">'
			. wfMessage( 'user-sandbox' )->escaped() . '</a></li>';
