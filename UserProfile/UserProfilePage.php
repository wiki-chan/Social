...
*/
		$wgOut->addHTML( $this->getSeriesInfo( $this->user_name ) );
		$wgOut->addHTML( $this->getCharInfo( $this->user_name ) );
		$wgOut->addHTML( $this->getSeiyuuInfo( $this->user_name ) );
		//Activity
		...
		*/
		...
		if ( $this->isOwner() ) {
			...
			$output .= 
				...
				'<li class="type3"><a class="view-watchlist" href="' . $editwatchlist->escapeFullURL() . '"><span class="caption">' . wfMsg( 'user-watchlist' ) . '</span><span class="icon watchlist"></span></a></li>';
			*/
			$output .= //$wgLang->pipeList( array(
//				'<a href="' . htmlspecialchars( $update_profile->getFullURL() ) . '">' . wfMessage( 'user-edit-profile' )->escaped() . '</a>' .
				'<li class="type2"><a href="' . htmlspecialchars( $upload_avatar->getFullURL() ) . '"><span class="caption">' . wfMessage( 'user-upload-avatar' )->escaped() . '</span><span class="icon avatar"></span></a></li>' .
				'<li class="type1"><a href="' . htmlspecialchars( $view_relationship_requests->getFullURL() ) . '"><span class="caption">' . wfMsg( 'user-requests-title' ) . '</span><span class="icon relationship"></span></a></li>' .
				'<li class="type3"><a href="' . htmlspecialchars( $watchlist->getFullURL() ) . '"><span class="caption">' . wfMessage( 'user-watchlist' )->escaped() . '</span><span class="icon watchlist"></span></a></li>' .
				'';
			//) );
		} elseif ( $wgUser->isLoggedIn() ) {
			if ( $relationship == false ) {
				$output .= //$wgLang->pipeList( array(
					'<li class="type2"><a href="' . htmlspecialchars( $add_relationship->getFullURL( 'user=' . $user_safe . '&rel_type=1' ) ) . '" rel="nofollow"><span class="caption">' . wfMessage( 'user-add-friend' )->escaped() . '</span><span class="icon addfriend"></span></a></li>'.
//					'<a href="' . htmlspecialchars( $add_relationship->getFullURL( 'user=' . $user_safe . '&rel_type=2' ) ) . '" rel="nofollow">' . wfMessage( 'user-add-foe' )->escaped() . '</a>'.
				...
				$output .= //$wgLang->pipeList( array(
					'<li class="type2"><a href="' . htmlspecialchars( $remove_relationship->getFullURL( 'user=' . $user_safe ) ) . '"><span class="caption">' . wfMessage( 'user-remove-friend' )->escaped() . '</span><span class="icon removefriend"></span></a></li>'.
					'';
//					) );
			...
			global $wgUserBoard;
			if ( $wgUserBoard ) {
				$output .= '<li class="type3"><a href="' . htmlspecialchars( $send_message->getFullURL( 'user=' . $wgUser->getName() . '&conv=' . $user_safe ) ) . '" rel="nofollow">' .
					'<span class="caption">' . wfMessage( 'user-send-message' )->escaped() . '</span><span class="icon sendmessage"></span></a>';
...
/*			$output .= '<a href="' . htmlspecialchars( $give_gift->getFullURL( 'user=' . $user_safe ) ) . '" rel="nofollow">' .
		...
		$output .= '<li class="type2"><a href="' . htmlspecialchars( $sandbox->getFullURL() ) . '" rel="nofollow"><span class="caption">' . wfMessage( 'user-sandbox' )->escaped() . '</span><span class="icon sandbox"></span></a></li>';
		$output .= '<li class="type1"><a href="' . htmlspecialchars( $contributions->getFullURL() ) . '" rel="nofollow"><span class="caption">' . wfMessage( 'user-contributions' )->escaped() . '</span><span class="icon contributions"></span></a></li>';
	...
	/**
	 * Get the series info for a given user.
	 *
	 * @param $user_name String: user name whose custom info we should fetch
	 * @return String: HTML
	 * @author 페네트-
	 */
	function getSeriesInfo( $user_name ) {
		global $wgUser, $wgUserProfileDisplay;

		# 먼저 데이터가 없다면 데이터를 먼저 불러옴
		$this->initializeProfileData( $user_name );
		$profile_data = $this->profile_data;

		# 데이터가 있는지 검사한다
		$joined_data = $profile_data['series_1'] . $profile_data['series_2'] .
						$profile_data['series_3'] . $profile_data['series_4'] . $profile_data['series_5'];
		...
		if ( $joined_data ) {
			...
			<ul class="profile-info-container">' .
				...
				$this->getProfileSection( '', $profile_data['series_3'], false, true ) .
				$this->getProfileSection( '', $profile_data['series_4'], false, true ) .
				$this->getProfileSection( '', $profile_data['series_5'], false, true ) .
			'</ul>';
		} else {
			$user_message = '아직 좋아하는 작품이 없네요.';
			if ( $wgUser->getName() == $user_name ) $user_message .= ' 새로 등록해주세요.';
	...
	/**
	 * Get the character info for a given user.
	 *
	 * @param $user_name String: user name whose custom info we should fetch
	 * @return String: HTML
	 * @author 페네트-
	 */
	function getCharInfo( $user_name ) {
		global $wgUser, $wgUserProfileDisplay;
		...
		# 데이터가 있는지 검사한다
		$joined_data = $profile_data['character_1'] . $profile_data['character_2'] .
						$profile_data['character_3'] . $profile_data['character_4'] . $profile_data['character_5'];
		...
		if ( $joined_data ) {
			...
			<ul class="profile-info-container">' .
				...
				$this->getProfileSection( '', $profile_data['character_3'], false, true ) .
				$this->getProfileSection( '', $profile_data['character_4'], false, true ) .
				$this->getProfileSection( '', $profile_data['character_5'], false, true ) .
			'</ul>';
		} else {
			$user_message = '아직 좋아하는 캐릭터가 없네요.';
			if ( $wgUser->getName() == $user_name ) $user_message .= ' 새로 등록해주세요.';
	...
	/**
	 * Get the seiyuu info for a given user.
	 *
	 * @param $user_name String: user name whose custom info we should fetch
	 * @return String: HTML
	 * @author 페네트-
	 */
	function getSeiyuuInfo( $user_name ) {
		global $wgUser, $wgUserProfileDisplay;
		...
		# 데이터가 있는지 검사한다
		$joined_data = $profile_data['seiyuu_1'] . $profile_data['seiyuu_2'] .
						$profile_data['seiyuu_3'] . $profile_data['seiyuu_4'] . $profile_data['seiyuu_5'];
		...
		if ( $joined_data ) {
			...
			<ul class="profile-info-container">' .
				...
				$this->getProfileSection( '', $profile_data['seiyuu_3'], false, true ) .
				$this->getProfileSection( '', $profile_data['seiyuu_4'], false, true ) .
				$this->getProfileSection( '', $profile_data['seiyuu_5'], false, true ) .
			'</ul>';
		} else {
			$user_message = '아직 좋아하는 성우가 없네요.';
			if ( $wgUser->getName() == $user_name ) $user_message .= ' 새로 등록해주세요.';
	...
	/**
	 * Get the user info for a given user.
	 *
	 * @param $user_name String: user name whose custom info we should fetch
	 * @return String: HTML
	 * @author 페네트-
	...
	function getUserInfo( $user_id, $user_name ) {
		...
		//TODO: function getPersonalInfo 함수 참고할 것. by 페네트
