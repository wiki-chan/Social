...
class SpecialUpdateProfile extends UnlistedSpecialPage {
		...
		if ( $wgRequest->wasPosted() ) {
			...
			switch( $section ) {
				case 'basic':
					...
					# Email 저장 및 Real_name 저장 메소드 삭제 (by 페네트)
	...
	function saveProfileBasic( $user = null ) {
		...
		// 현재 user_id값을 가지고 user_profile에서 불러옴
		...
		$basicProfileData = array(
			...
			*/
			'up_whoami' => $wgRequest->getVal( 'whoami' )
		);
		$dbw->update(
			'user_profile',
...
*/
		...
		$usertitle = Title::newFromText($wgUser->getName(), NS_USER);
		/*
	...
	function displayPersonalForm( $user ) {
		...
		$s = $dbr->selectRow(
			...
			array(
				...
				*/
				'up_series1', 'up_series2', 'up_series3', 'up_series4', 'up_series5',
				'up_character1', 'up_character2', 'up_character3', 'up_character4', 'up_character5',
				'up_seiyuu1', 'up_seiyuu2', 'up_seiyuu3', 'up_seiyuu4', 'up_seiyuu5'
			),
			array( 'up_user_id' => $wgUser->getID() ),
			__METHOD__
		);
		...
		if ( $s !== false ) {
			...
			$drinks = $s->up_drinks;
			*/
			$character1 = $s->up_character1;
			$character2 = $s->up_character2;
			$character3 = $s->up_character3;
			$character4 = $s->up_character4;
			$character5 = $s->up_character5;
			$seiyuu1 = $s->up_seiyuu1;
			$seiyuu2 = $s->up_seiyuu2;
			$seiyuu3 = $s->up_seiyuu3;
			$seiyuu4 = $s->up_seiyuu4;
			$seiyuu5 = $s->up_seiyuu5;
			$series1 = $s->up_series1;
			$series2 = $s->up_series2;
			$series3 = $s->up_series3;
			$series4 = $s->up_series4;
			$series5 = $s->up_series5;
		}
