...
class UserProfile {
	...
	public function getProfile() {
		...
		if ( $data ) {
			...
			*/
			...
			$profile['series_3'] = isset( $row->up_series3 ) ? $row->up_series3 : '';
			$profile['series_4'] = isset( $row->up_series4 ) ? $row->up_series4 : '';
			$profile['series_5'] = isset( $row->up_series5 ) ? $row->up_series5 : '';
			$profile['character_1'] = isset( $row->up_character1 ) ? $row->up_character1 : '';
			$profile['character_2'] = isset( $row->up_character2 ) ? $row->up_character2 : '';
			$profile['character_3'] = isset( $row->up_character3 ) ? $row->up_character3 : '';
			$profile['character_4'] = isset( $row->up_character4 ) ? $row->up_character4 : '';
			$profile['character_5'] = isset( $row->up_character5 ) ? $row->up_character5 : '';
			$profile['seiyuu_1'] = isset( $row->up_seiyuu1 ) ? $row->up_seiyuu1 : '';
			$profile['seiyuu_2'] = isset( $row->up_seiyuu2 ) ? $row->up_seiyuu2 : '';
			$profile['seiyuu_3'] = isset( $row->up_seiyuu3 ) ? $row->up_seiyuu3 : '';
			$profile['seiyuu_4'] = isset( $row->up_seiyuu4 ) ? $row->up_seiyuu4 : '';
			$profile['seiyuu_5'] = isset( $row->up_seiyuu5 ) ? $row->up_seiyuu5 : '';

			$profile['whoami'] = isset( $row->up_whoami ) ? $row->up_whoami : '';
	...
	static function getEditProfileNav( $current_nav ) {
		...
		$output = '<nav class="profile-tabs"><ul>';
		...
		foreach ( $lines as $line ) {
			if ( strpos( $line, '*' ) !== 0 ) {
				...
				$output .= '<li class="' . ( ( $current_nav == $link_text ) ? 'active' : '' ) . '">';
