...
//var_dump($res);
		$message = $res->ub_user_name_from . "(ë¡œë";
		if ($res->ub_type == 1) $message .= "ë¹„ë ";
		$message .= "ë©”ì‹œì§€ë¥ë°›ì•˜µë‹ˆ;
	...
	public function sendBoardMessage( $user_id_from, $user_name_from, $user_id_to, $user_name_to, $message, $message_type = 0 ) {
		...
		$id = $dbw->nextSequenceValue('user_board_id');
	 ...
	 * @return Integer: amount of new messages
	 */
	static function getNewMessageCountDB( $user_id, $ub_old_id ) {
		global $wgMemc;

		//if ( !$ub_old_id ) return '';
		// ì§ì „ touchedœê°„ ê¸°ì¼ë¡œ ë©”ì‹œì§€ë¥ê°€¸ì˜´(ì£ UTC 0 œê°„ œêµ­ œê°„¼ë¡  +9h ´ì£¼´ì•¼ 
	...
	public function getUserBoardMessages( $user_id, $user_id_2 = 0, $limit = 0, $page = 0 ) {
		...
		$this->clearNewMessageCount($user_id);
